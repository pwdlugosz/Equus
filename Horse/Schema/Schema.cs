...
namespace Equus.Horse
{
    ...
    public sealed class Schema
    {
        ...
        /// <summary>
        /// Returns the number of bytes a data record takes up, excluding meta data
        /// </summary>
        ...
        /// <summary>
        /// The length of the data + the meta data size
        /// </summary>
        public int RecordLength
        {
            get
            {
                int data = this.DataLength;
                int meta = this.Count * 2;
                return data + meta;
            }
        }
        ...
        public bool Check(Record R, bool FixAffinity)
        {
            ...
            for (int i = 0; i < R.Count; i++)
            {
                ...
                else if (this.ColumnSize(i) <= R[i].Size && this.ColumnAffinity(i) == CellAffinity.STRING)
                    R._data[i].STRING = R[i].STRING.Substring(0, this.ColumnSize(i));
                else if (this.ColumnSize(i) <= R[i].Size && this.ColumnAffinity(i) == CellAffinity.BLOB)
                {
