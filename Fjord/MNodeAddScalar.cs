...
{
    public sealed class MNodeAddScalar : MNode
    {

        private int _Association = 0; // 0 == left (A * B[]), 1 == right (B[] * A)
        private FNode _expression;

        public MNodeAddScalar(MNode Parent, FNode Expression, int Association)
            : base(Parent)
        {
        ...
        {
            MNode node = new MNodeAddScalar(this.ParentNode, this._expression.CloneOfMe(), this._Association);
